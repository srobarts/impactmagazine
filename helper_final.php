...
function filter($rows2, $menge, $model) {
	...
	//get the filter variable from javascript
	$location_filter = JRequest::getVar('location_filter', '');
	//separate out the wildcard characters, prepare for SQL
	$wildcard = '*';
	$sql_wildcard = '%';
	$location_filter = str_replace($wildcard,$sql_wildcard,$location_filter);
	
	JRequest::setVar('location_filter', $location_filter); //replace javascript variable with filter result
	...
	$querypart_1 = "SELECT * FROM ". getTableName(). "  WHERE ";
	$querypart_2 = $rows2[0]->realname . ' LIKE "'. "%". htmlentities($filternach, ENT_QUOTES, 'utf-8') . "%". '"'. " AND ";
	$querypart_3 = $rows2[1]->realname . ' LIKE "'. "%". htmlentities($event_filter, ENT_QUOTES, 'utf-8') . "%" . '"' . " AND ";
	$querypart_4 = $rows2[1]->realname . ' LIKE "'. "%". htmlentities($keyword_filter, ENT_QUOTES, 'utf-8') . "%" . '"' . " AND ";
	$querypart_5 = $rows2[2]->realname . ' LIKE "'. "%". htmlentities($location_filter, ENT_QUOTES, 'utf-8') . "%" . '"';
	$querypart_6 = " ORDER BY ". $ordering;
		  
	$ryn = JRequest::getVar('resetyesno', 1, 'get');  //get reset variable from javascript
	$query = '';
	
	if ((($menge) != 0) && ($ryn && ((int) $model->getConfig('filter', $number) == 0) || $filternach != '' || $event_filter != '' || $city_filter != '' || $keyword_filter != '')){
		$query = $querypart_1 . $querypart_2 . $querypart_3 . $querypart_4 . $querypart_5 . $querypart_6; //get complete query string
	}
